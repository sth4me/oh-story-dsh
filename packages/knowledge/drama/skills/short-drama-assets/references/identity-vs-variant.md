...
每条决定至少展示：来源出现记录、候选、选择、稳定证据、差异、原因、有效范围、
下游影响和创作者接受记录。
...
`creator_acceptance.decision_ref` 必须指向一条创作者决策记录，其 `record_id` 以 `CD-`
开头——检查器据此判定，不合规会直接拦下发布。指向剧本块或资产记录本身的 `decision_ref`
把「已接受」绑到了一条从未记录过创作者决策的东西上。决策记录本身由创作者决策环节发布在 `创作者决策/` 下，本阶段只引用它。

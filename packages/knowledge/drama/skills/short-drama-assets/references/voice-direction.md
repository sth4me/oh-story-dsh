...
`artifact_ref` 只说明绑定了哪条记录，**不能证明有人真的听过这段音频**（`AST-09`）。
每条绑定写清证据到了哪一步：
...
参考音频与授权说明属于创作者输入，放在 `输入/` 下：在文件首行的 `sources` 里按整文件
声明一次，`artifact_ref` 写 `{"src":"guhe-audition-02"}`——指向整份媒体，不带
`record_id`。**媒体本身不进 设定集，也不进交付包**，只进声明与引用。
...
## voice-casting.md 的渲染规则

`设定集/voice-casting.md` 是已接受 `voice_direction` 的**派生文本**，交给配音导演、
选角或克隆操作者使用。它由已接受记录重新生成，是缓存，不是第二份权威——手改它不会改变
任何身份，下一次渲染就会被覆盖。

每个角色一段，固定四块：

1. **参考绑定**：哪条参考、`role`、可借用什么、不可借用什么、`admission_status`；
   没有参考时写「待选型」并列出选型判据；
2. **选型判据**：三到五条，每条带反例；
3. **区分说明**：和谁最容易混、用哪条区分；
4. **专名发音**：已接受的那一份拼法。

不写入任何服务商参数、模型名、接口字段、任务 ID 或网址——记录要比任何一次调用活得久。

**资产阶段不生成音频，也不调用任何语音、合成或克隆服务**。 文本绑定只能保证角色、
声音方向与引用关系没有被改写；它不能承诺任何真实的合成结果、音色复现或口型同步。
记录先落进文件、由创作者确认；需要实际 TTS 时再交给 `$short-drama-produce`，由它展示
精确任务预览并取得本次生产的明确确认。
...
| 失败 | 症状 | 怎么改 |
|---|---|---|
| 用文字冒充音色 | 没有参考，却写了一整段音色描述当身份 | 保持 `reference: null` 并标「待选型」，判据归判据 |
| 参考的情绪进了身份 | 借用范围里出现「这条里的哽咽」 | 移进 `must_not_control` |
| 录音空间被当成音色 | 身份里写了混响、房间感、话筒距离 | 那属于那次录音，删掉 |
| 声称听过却没听过 | `admission_status` 写着已检查，却没有观察记录 | 退回 `unverified` 并列风险 |
| 两个角色撞声 | 两条参考互换名字也说得通 | 补区分判据；补不出就换参考 |
| 发音两套拼法 | 同一专名在两处不同 | 回到已接受的那一份 |

...
| 内容 | 字段用途 | 写法 |
|---|---|---|
| 剧本段和场次位置 | `source_ref` | `src` 指向首行声明的剧本快照，加 `record_id` 与需要的 `field`；不复制整段权威文本 |
| 剧本明示的可见事实 | `derived_projection` | 简短摘义，并指回准确字段 |
| 拟绑定的资产 | `derived_annotation` | 只保留提案；资产决定记录反向引用本条出现记录，并拥有最终决定 |
| 连续性变化 | `source_ref` | `src` 指向资产技能负责的变化记录产物，加 `record_id` 与需要的 `field` |
